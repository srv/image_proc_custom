...
void ScaleNodelet::connectCb()
{
  ...
  {
    if ( (current_inputs_.size()>0) || subs_info_.getSubscriber() )
      cb_connection_.disconnect();
    const int num_inputs = inputs.size();
    if ( (num_inputs > 0) || publ_info_.getNumSubscribers() > 0 )
    {
      ...
      {
        case 0 :
          cb_connection_ = subs_info_.registerCallback(
                  boost::bind(&ScaleNodelet::imagesCb, this, _1, null_ptr, null_ptr, null_ptr, null_ptr) );
          break;
        case 1 :
